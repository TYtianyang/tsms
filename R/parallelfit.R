parallel.fit = function(X,W.list,para_panel,U,k,multicore=F){
  ...
  if (!multicore){
    ...
    for (i in 1:nrow(para_panel)){
      ...
      m = FSARMA.fit(X,U,p,q,S1,S2,W.temp,k)
      para_panel[i,6] = m$aic
    }
